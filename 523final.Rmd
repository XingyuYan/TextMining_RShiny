...
#{
  ...
  write.table(tbl2,"textmining.txt",sep="\t",row.names=FALSE)
#  return(tbl2)
...
## Plot.1 hist for most 50 freq
library(ggplot2)
df = tbl2[1:50,]
names = as.matrix(tbl2[1:50,1],col = 1)
freq = as.matrix(tbl2[1:50,2],col = 1)
ggplot(df, aes(names,freq)) + 
...
## Plot.2 word_cloud
library(wordcloud)
max_words = 150  #if want the most 150 freq words, user can choose this value themselves in shiny
namesW = as.matrix(tbl2[,1],col = 1)
freqW = as.matrix(tbl2[,2],col = 1)
wordcloud(names,freq,max.words = max_words,scale=c(5, .1), colors=brewer.pal(6, "Dark2")) 
